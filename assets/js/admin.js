...
(function ($) {
    ...
    function doParse() {
        ...
        $.ajax({
            ...
            success: function (resp) {
                ...
                if (!resp.success) {
                    alert('Error: ' + (resp.data?.message || 'Unknown error')); return;
                }
                ...
                // Show language picker if multilingual plugin is active
                if (resp.data.ml_active) {
                    $('#ymi-lang-picker').show();
                }
            ...
            ,
            error: function () {
                $spinner.removeClass('is-active'); $('#ymi-btn-upload').prop('disabled', false); alert('AJAX request failed.');
            }
        }
    ...
    function doPreview() {
        ...
        const lang = $('#ymi-lang').length ? $('#ymi-lang').val() : '';
        const transientKey = $('#ymi-upload-form').data('transient_key');

        if (!mapUrl || !mapTitle || !mapDesc) {
            alert('Please map all three columns.'); return;
        }
        ...
        $.ajax({
            ...
            data: {
                action: 'ymi_preview', nonce: ymi_ajax.nonce, map_url: mapUrl, map_title: mapTitle, map_desc: mapDesc, transient_key: transientKey, lang: lang
            }
    ...
    /**
 * Build a git-diff style table cell showing old (red) and new (green) values
 * stacked vertically with −/+ prefixes.
 */
    function buildDiffCell(oldVal, newVal, changed) {
        var $td = $('<td>').addClass('ymi-diff-cell');
        var hasOld = oldVal && oldVal.trim();
        var hasNew = newVal && newVal.trim();

        if (!changed && hasOld) {
            // Unchanged — show once normally
            $td.append($('<span>').addClass('ymi-diff-unchanged').text(oldVal));
        }

        else if (!hasOld && hasNew) {
            // Fully new value (was empty before)
            $td.append($('<span>').addClass('ymi-diff-only-new').text(newVal));
        }

        else if (hasOld && !hasNew) {
            // Removal only (value being cleared)
            $td.append($('<span>').addClass('ymi-diff-old').text(oldVal));
        }

        else if (hasOld && hasNew && changed) {
            // Changed — show old above, new below
            $td.append($('<span>').addClass('ymi-diff-old').text(oldVal));
            $td.append($('<span>').addClass('ymi-diff-new').text(newVal));
        }

        else {
            // Both empty
            $td.append($('<span>').addClass('ymi-diff-empty').text('(empty)'));
        }
    ...
    function renderPreview(data) {
        ...
        $('#ymi-preview-stats').html('<span class="ymi-stat"><strong>' + preview.length + '</strong> rows total</span>' + '<span class="ymi-stat" style="color:#155724;"><strong>' + foundCount + '</strong> pages matched</span>' + (notFoundCount > 0 ? '<span class="ymi-stat" style="color:#c5221f;"><strong>' + notFoundCount + '</strong> not found</span>' : ''));
        ...
        preview.forEach(function (entry, i) {
            ...
            $row.append($('<td>').addClass('check-column').append($('<input>', {
                type: 'checkbox', value: i, checked: isFound
            }
            ...
            if (!isFound) {
                ...
                var badgeText = entry.entity_type === 'home' ? 'home' : entry.entity_type === 'ptarchive' ? (entry.post_type || 'archive') : entry.entity_type === 'taxonomy' ? (entry.taxonomy || 'taxonomy') : (entry.post_type || 'page');
                pageLabel += ' <span class="ymi-badge ymi-badge-found">' + badgeText + '</span>';
            }
            // Language badge
            if (entry.lang) {
                pageLabel += ' <span class="ymi-badge ymi-badge-lang">' + entry.lang.toUpperCase() + '</span>';
            }

            $row.append($('<td>').html(pageLabel));
            $row.append($('<td>').text(entry.url));
            $row.append(buildDiffCell(currentTitle, newTitle, titleChanged));
            $row.append(buildDiffCell(currentDesc, newDesc, descChanged));
            $tbody.append($row);
        }
    ...
    $('#ymi-btn-import').on('click', function () {
        ...
        $('#ymi-preview-tbody input[type="checkbox"]:checked').each(function () {
        ...
        if (selected.length === 0) {
            alert('Please select at least one row to import.'); return;
        }

        const importKey = $('#ymi-import-results').data('import_key');

        if (!confirm('Are you sure you want to update Yoast SEO metadata for ' + selected.length + ' page(s)? This will overwrite existing values.')) {
            return;
        }
        ...
        $.ajax({
            ...
            url: ymi_ajax.ajax_url, type: 'POST',
            data: {
                action: 'ymi_import', nonce: ymi_ajax.nonce, import_key: importKey, selected: JSON.stringify(selected)
            }
            ...
            success: function (resp) {
                ...
                if (!resp.success) {
                    $results.removeClass('ymi-results-success').addClass('ymi-results-error').html('<strong>Error:</strong> ' + (resp.data?.message || 'Unknown error'));
                    return;
                }
                ...
                if (data.skipped > 0) {
                    html += data.skipped + ' row(s) skipped (not selected).<br>';
                }
            ...
            ,
            error: function () {
                $spinner.removeClass('is-active'); $('#ymi-btn-import').prop('disabled', false); alert('AJAX request failed.');
            }
        }
